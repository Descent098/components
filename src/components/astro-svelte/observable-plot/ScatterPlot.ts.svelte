<script lang="ts">
    ...
    const {
        ...
        dotColor=undefined,      // The color of the dots (will be overridden by colorAttribute if set)
        ...
        valuesOnDots=false,        // Whether to show the y-value of the dot over top of the dot
        legend=true,              // Whether to show the color legend (only applies when z-axis is specified)
        colorAttribute=undefined, // The attribute to color the graph by
    } = $props();
    ...
    onMount(async ()=>{
        ...
        y:{label:yAxisLabel},
        marks: [
            Plot.dot(data, {x: xAxisDataField, y: yAxisDataField, sort: sort? {x: "y", reverse: sortDescending}:undefined, stroke:colorAttribute?colorAttribute:dotColor, tip:tooltips, }),
            domain ? Plot.ruleY(domain):Plot.ruleY([0]),
            Plot.axisX({marginBottom:40}),

            valuesOnDots?Plot.text(data, {x: xAxisDataField, y: yAxisDataField, text: (d) => (d[yAxisDataField]), dy: -6, lineAnchor: "bottom"}):undefined,
        ],
        style: styleOverrides,
        color: colorConfig? colorConfig:colorAttribute?{legend:legend, label:colorAttribute}:undefined
        })
        initialized = true
        updateComponent()
  })
