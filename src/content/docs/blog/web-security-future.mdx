...
cover:
  ...
  image: ../../../assets/blog/web-security-future.png
---
...
UPDATE (Feb 4th 2026): A little while after writing this peice there was a good example of the positive effect in the recent vulnerability disclosures in OpenSSL by [AISLE](https://aisle.com/) using AI. Essentially the managed to find 12 CVE's in OpenSSL and 5 CVE's in the cURL project[^8]. So, there are concrete examples of the good it can do in the ecosystem.
...
As was seen in the [cURL](#cURL) example from earlier we just have people that are shipping and sending things without review. Charitably this could be chalked up to laziness, but more likely it's the inability to validate. This problem is exacerbated by the AI maximalism of the industry that is being perpetuated by AI companies, and other major platforms like github. Github has recently for example published an article called  [*How to create issues and pull requests in record time on GitHub*](https://github.blog/developer-skills/github/how-to-create-issues-and-pull-requests-in-record-time-on-github/), nothing in this article has any regard for the quality of the code, security considerations, or automation to help combat potential spam.

In the 12 hours since I wrote the last paragraph [TL;Draw](https://www.tldraw.com/), an open source (ish now) whiteboarding application has [closed contributions](https://tldraw.dev/blog/stay-away-from-my-trash) due to AI generated code. There are other similar situations where not only bug bounties are being killed, but open contribution policies are being changed:

- A few months ago the [zig language](https://ziglang.org/) [banned the use of LLM's](https://www.techzine.eu/news/devops/136914/zig-project-leaves-github-due-to-excessive-ai/), and ended up leaving github transitioning to [codeberg](https://codeberg.org/) after citing issues with suspected vibe-coded solutions in [github actions](https://www.theregister.com/2025/12/02/zig_quits_github_microsoft_ai_obsession/). Github is owned by microsoft and the trend of software decline at microsoft is something [I wrote about previously](https://kieranwood.ca/tech/blog/microsoft-please/), and since then has been [getting worse](https://www.windowscentral.com/microsoft/windows-11/windows-11-apps-like-notepad-arent-loading-what-is-error-code-0x803f8001-and-how-d), and [worse](https://www.youtube.com/watch?v=DsTlxRKkPyY)
- The [ghostty](https://ghostty.org/) terminal has [banned "drive-by" AI contributions](https://github.com/ghostty-org/ghostty/pull/10412), meaning you can only use it to assist on agreed upon issues

You may think this indiffernce to risk just effects the open-source world, but this attitude is incredibly prevalent everywhere. Annecdotally I've had 6 events this month where various different people have confided in me that the **overwhelming majority** of their work is vibe-coded. Some of which are people that oversaw the transition from one platform and tech stack to another, all while not writing a single line of code (or learning anything about their new system). 

Meanwhile other more AI maximalists are suggesting [replacing dependencies with vibe-coded solutions](https://www.youtube.com/watch?v=u9P3CKwtRnM). A quintessential example of this is [levelsIO](https://x.com/levelsio). Someone who has created many businesses around letting AI write the majority of the code including a vibe coded game with major RCE exploits[^7]. 

### The Lying Elephant in the Room

While it's not guarenteed that if any of this code was hand-written it would certainly have been better. There have certainly been mistakes made by people programming. The problem with this attitude is that it throws away all pretense of problem-solving in exchange for speed. This tradeoff can be fine when projects do not matter, however for consequential projects deference needs to be paid to slowing down and doing things properly. This was an issue even before AI, but now it permeates some of the largest companies around (microsoft, google, etc.), who are just rushing out products, services, and updates that don't work at all consistently. 

## The Boring Truth

The reality is that for better or worse AI tools are here. There are ways to use them appropriately, but the overall ROI in a security sense is yet to be seen over the long term. On the one hand the slop being propogated will have many security concerns, on the other the AI detection of security concerns is improving. The question of which will win out when amortized over the coming years is unknown. While I can optimistically hope that it will be used appropriately, as we've seen it's not looking any more likely than the negative outcomes so far as we've seen with many of the promises made in our glorious AI future.
...
[^8]: https://www.lesswrong.com/posts/7aJwgbMEiKq5egQbd/ai-found-12-of-12-openssl-zero-days-while-curl-cancelled-its
